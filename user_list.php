  ...
  <div class="row">
    <div class="col-xs-12">
      <table summary="GYM USER TABLE" class="table table-bordered table-hover dt-responsive">
        ...
        if($data = mysqli_query($conn,"select * from users")){
            while($row = mysqli_fetch_assoc($data)) {
                echo "<tr><td>".$row["name"].'</td><td><a onclick="user_edit(this)">'.$row["email"]."</a></td><td>".$row["mobile"]."</td><td>".$row["DOB"]."</td><td>".$row["father_name"]."</td></tr>";
            }
        }
        else{
            echo "error";
